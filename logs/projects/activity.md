...
| :--- | :--- | :--- | :--- | :--- |
| **2026-09-24** | **flowerpot-app (花盆)** | ssh / `main` | **割草机充电中闪电动效**：按 Figma 746:2657 挪进电量环、1.4s 呼吸循环（稿子仅图层名给参数，无 GIF / 原型数据）。 | 已推送 `d11baed`（未编译） |
| **2026-09-24** | **album-app (相册APP)** | ssh / `main` | **iOS 包隐藏微信快捷登录**（审核口径，安卓不变）：一处开关 `weChatLoginHiddenOnThisApp`，登录页微信按钮 iOS 不画，补用例。 | 已推送 `0c4220c`（未编译） |
| **2026-09-24** | **flowerpot-app (花盆)** | ssh / `main` | **按 Figma 76 节点逐页核对布局**：标题栏 56→46、返回键格宽 82，约 35 页顶部按「稿子 y−90」；按钮落点、插图尺寸、割草机设置子页/弹窗、搜索与配网页、状态页控件等按稿子；字号未改（待产品定）。 | 已推送 `1cd407a`（未编译） |
| **2026-09-23** | **flowerpot-app (花盆)** | ssh / `main` | **更多设备信息改版**：花盆去 APP版本/MAC/两个 UUID，新增设备型号（涂鸦产品名）；割草机「设备参数」共用该页（固件版本、设备型号、devId、mac、硬件版本），删 SN 与旧页。割草机硬件版本无 DP 空着。 | 已推送 `52472a5`（未编译） |
| **2026-09-23** | **flowerpot-app (花盆)** | ssh / `main` | **花盆首页背景与坐标对齐 Figma 156:4304**：背景按状态栏与屏宽反推位置，让光圈对准植物（375/44 时与稿子一致）；前景顶部 20→16、养护条间距 18→22。 | 已推送；未编译 / 未真机 |
| **2026-09-23** | **flowerpot-app (花盆)** | ssh / `main` | **全局缩放撤回**（相册 App 无全局缩放且正常，改逐页按 Figma 核对）；「我的」页横线调整。 | 已推送 `24703f3` `c7b5b8a` |
...
| **2026-09-23** | **flowerpot-app (花盆)** | ssh / `main` | **割草机下发通道 + 固件提醒跳转 + 首页 Figma 重排**：在线指令强制走 Wi-Fi（Internet 模式）、离线主动降级蓝牙（Auto）；立刻更新先跳设置页再弹升级框（花盆同理）；蓝牙改先扫描再连；首页按 784:3844 余量只给电量环。 | 已推送；未编译 / 未真机 |
| **2026-09-23** | **flowerpot-app (花盆)** | ssh / `main` | **割草机五项**：计划图夜间色块不被挡、首页撑满首屏去光晕、设备名统一字重、信号 -90、蓝牙连不上根因（SDK 静默跳过：手机蓝牙关 / 产品无蓝牙能力位）提前判并记日志，只连蓝牙也能控制。 | 已推送 `d451bad`；未编译 / 未真机 |
| **2026-09-23** | **flowerpot-app (花盆)** | ssh / `main` | **割草机电量环下文字对齐**：状态词与百分比原为 Row 里两个 Text 各自居中（中文 / 数字字体基线不同而错开），改为同一 Text.rich 共用基线。 | 已推送；未编译 / 未真机 |
| **2026-09-23** | **flowerpot-app (花盆)** | ssh / `main` | **割草机蓝牙连接补参数与诊断**、检查中按钮换转圈、去掉设置页问号。蓝牙连不上疑为固件未开本地控制，待真机日志。 | 已推送；未编译 / 未真机 |
| **2026-09-23** | **flowerpot-app (花盆)** | ssh / `main` | **割草机固件升级提醒同花盆**：设备升级行红点，进割草机页面按升级方式弹提醒 / 强制升级框，立刻更新直接发起。 | 已推送 `e241c05`；未编译 / 未真机 |
| **2026-09-23** | **flowerpot-app (花盆)** | ssh / `main` | **固件升级准备中文案**：去掉「开始升级后会显示进度」，花盆与割草机共用同一弹框。 | 已推送 |
| **2026-09-23** | **flowerpot-app (花盆)** | ssh / `main` | **割草机蓝牙图标与控制前蓝牙检查**：图标按涂鸦分通道状态亮、进页面自动连蓝牙；四颗按钮先连蓝牙再查信号（弱于 -80 拦），不满足不下发。 | 已推送；未编译 / 未真机 |
| **2026-09-23** | **flowerpot-app (花盆)** | ssh / `main` | **修打包编译错误**（mower_firmware 缺 TuyaException 导入、可空泛型）；割草机蓝牙门禁需求用户暂缓，只记了 SDK 接口核对。 | 已推送 `14fe25a` |
| **2026-09-23** | **flowerpot-app (花盆)** | ssh / `main` | **割草机 DP18 改回协议 V2.3 位图**：0 = 正常黄条隐藏，有故障位才显示文案；撤回按编号那版。 | 已推送；未编译 / 未真机 |
| **2026-09-23** | **flowerpot-app (花盆)** | ssh / `main` | **割草机 DP18 按编号显示**：上报值即文案编号（0=左轮异常），没报过不显示故障；与协议位图口径冲突，待固件确认。 | 已推送；未编译 / 未真机 |
| **2026-09-23** | **flowerpot-app (花盆)** | ssh / `main` | **设备列表中文设备名不加粗**：系统中文字体无中等字重，w500 落到粗体；中文名改 w400，英文名不变。 | 已推送 `beed0cf`；未编译 / 未真机 |
| **2026-09-23** | **flowerpot-app (花盆)** | ssh / `main` | **割草机设置页补齐**：帮助中心一定用割草机产品 ID（原来不在 deviceIdList 时退回花盆的）；设备升级、删除设备同花盆；重新配网隐藏；计划页高度随屏幕自适应。 | 已推送 `e145aaa`；静态自检，未编译 / 未真机 |
| **2026-09-23** | **flowerpot-app (花盆)** | ssh / `main` | **割草机联调追改**：左三只在 DP5=paused 时禁用（待机能点自动）；DP101 逐字节解析临时窗口 + 日志；恢复默认接 V2.3 DP110；改名走涂鸦 + 后端 editUserProduct；计划无数据时首页缺省「暂无割草计划」、编辑框不再预填稿子时段。 | 已推送 `502865f` `5f0d7cf` `888c3b6`；静态自检，未编译 / 未真机 |
| **2026-09-23** | **flowerpot-app (花盆)** | ssh / `main` | **搜索页设备图标按产品区分**：割草机用割草机图标、花盆用花盆图标，认不出的退回蓝牙/热点图标；另核对割草机 6 条 DP 口径（全部已实现）与协议 20 个 DP（未接 111/116/119）。 | 已推送 `8eb5489`；静态自检，未编译 / 未真机 |
| **2026-09-23** | **minerals-admin (正矿)** | ssh / `feature-v1.8.4` | **国内销售列表新增按钮改名「新增国内销售」**，与国内采购对齐。 | 已推送 `9fbed47`；SFC 编译检查 |
| **2026-09-23** | **flowerpot-app (花盆)** | ssh / `main` | **割草机联调：打开联调日志、去掉全部假数据**：日志闸默认开；删静态入口、演示模式、模拟面板、稿子上的名称/型号/SN/计划/消息；名称读设备名、型号/SN 空着；消息通知改读设备 DP 历史 + 消息中心；「恢复默认」改只提示（协议无默认计划）。 | 已推送 `376fbd6`；静态自检，未编译 / 未真机 |
| **2026-09-23** | **minerals-admin (正矿)** | ssh / `1.8.4-deep-space-theme` | **星轨登录页选中节点尖角改气泡尾巴**：描边从圆圈接出、同底色、不单独发光，和圆圈连成一体。 | 已推送 `b804c79`；渲染台放大 + 像素核对 + build |
| **2026-09-23** | **minerals-admin (正矿)** | ssh / `1.8.4-deep-space-theme` | **快捷搜索弹框深色化**：深色规则原来挂在匹配不到的容器下（弹框在 body 上），结果区浅底浅字；改到最外层并按深空玻璃卡重做，加搜索图标 / Esc 提示，去掉输入框里的直角聚焦框。 | 已推送 `5deed6d`；渲染台四态截图 + build |
