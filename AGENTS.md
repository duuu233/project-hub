...
**没有特殊说明时，任务完成并验证通过后默认提交并推送。** 本轮改动过的每个仓库都要做，包括 Project Hub 自身：先按目标项目自身要求更新它的文档，提交到当前工作分支，push 前对该分支执行一次 `git pull --ff-only`，成功后再 push 到它已配置的 upstream。用户本次说明例外时按用户的说法办，例如“这次先改好并验证，不提交”“只推业务项目，Hub 先不动”。

1. 验证没通过、改动仍是半成品，或用户明确要求先看效果时不提交：说明当前状态，把改动留在工作区等用户决定。
