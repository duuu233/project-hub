...
## 任务完成后的默认动作：再 pull 一次，然后 push

**只要本轮动了代码，用户又没说不提交，就默认提交并推送。** 本轮改动过的每个仓库都要做，包括 Project Hub 自身：先按目标项目自身要求更新它的文档，提交到当前工作分支，push 前对该分支执行一次 `git pull --ff-only`，成功后再 push 到它已配置的 upstream。用户本次说明例外时按用户的说法办，例如“这次先改好并验证，不提交”“只推业务项目，Hub 先不动”。

1. **不提交的唯一理由是用户本次说了不提交**（“先改好别提交”“我先看看效果”）。**验证没跑通、本机缺工具链、没真机验过、改动只是一版待验证的实现——都不是扣着不推的理由**（2026-09-11 明确口径）。把没验证的部分如实写进提交信息和迭代记录，然后照常 push；需要先看效果时用户会自己说。
2. push 前的 `git pull --ff-only` 出现分叉、冲突、鉴权失败或远端不可达时停下报告，不自动 merge、rebase 或强推。
3. 分支没有配置 upstream 时不擅自选远端分支：说明情况，由用户决定是否 `push -u <remote> <branch>`。
4. 默认动作只到 push 为止。**创建 PR、合并、发布、部署和打 tag 仍需用户明确指令。** 首次初始化不自动创建提交。
5. 完成后逐个仓库报告改动、验证结果、分支和提交号，不把多仓库合并成一句“已推送”。

## 可写边界：SSH 环境下只许动 `/pgdata/pg/dh`（2026-09-17 用户明确，原则）

**在 SSH 开发机上、通过本 Hub 工作时，可写范围只有 `/pgdata/pg/dh/` 这一个文件夹。**
该目录之外的东西**是别人的**——尤其 `/pgdata/pg/work/`（另一套在用的工作目录，约 15GB，
内含多个 `.wt-*` git worktree，里面可能有未推送的分支）。

界线是**「读可以，写绝对不行」**：

- ✅ **允许只读借用**别人目录里的插件、工具包、依赖。例如跑 Playwright 截图时引
  `/pgdata/pg/work/.../node_modules/playwright-core`——只 import，不改动；产物写回 `dh` 下。
- ❌ **绝对不可操作**：不得在 `/pgdata/pg/dh/` 之外**新增、修改、删除**任何文件，不得移动或清理，
  也不得执行有副作用的命令（`install` / `build` / `git gc` / `checkout` / `clean` 等）。
- 磁盘告急**不构成**清理别人目录的理由。可以如实报告那边占了多少、问题根源在哪，
  但处理与否由用户决定，Agent 不动手。

临时产物一律写在 `/pgdata/pg/dh/project-hub/.codex-tmp/`（已 gitignore）。
这条与下面的「信息隔离」是两件事：那条管**不把 Hub 的痕迹写进别人能看到的仓库**，
这条管**不去碰别人的文件**。两条都是硬规矩。
...
- Hub 的先 pull、完成后默认提交并推送等统一约定持续适用；具体项目文档不能被 Hub 摘要替代。若要求冲突，明确说明并按指令优先级处理。
