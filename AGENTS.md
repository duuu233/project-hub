...
对 `/pgdata/pg/dh/` 之外的路径：

- **不得创建、修改、删除、移动或清理**任何文件；
- **不得在其中执行有副作用的命令**（`install` / `build` / `git gc` / `checkout` / `clean` 等）；
- 磁盘告急、缺依赖、想借个工具包——**都不构成越界的理由**。可以如实报告那边占了多少空间、
  问题根源在哪，但处理与否由用户决定，Agent 不动手。
- 连只读依赖也不要建立：需要某个工具（如 Playwright）就在 `dh` 下自己装一份，
  不要引用别人目录里的 `node_modules`。
