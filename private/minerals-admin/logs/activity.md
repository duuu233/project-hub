...
### 2026-09-24：深空特效改造——登录基站联动 + 蓝锆石、数据看板、业务页数据到达（`1.8.4-deep-space-theme`，`e907f61` / `3404f8f` / `aa7d6d8`）
- 用户要求：只改深空分支；登录页切系统时左侧插画基站逐个点亮、全部点亮后中间升起矿石（高清、高级；「不要砖石，查看系统哪个矿石最合适」）；默认首页（数据看板）更科技更酷炫；其它页面「数据传递、硬核、数据都是智能化出来」。「效果不满意我再回退」→ 三块各自单独一个提交，可分别 revert。
- **矿石选型**：蓝锆石。平台主营钛锆（行情追钛精矿、海关数据是钛锆进出口、融资品种有锆矿 / 锆英砂），锆英砂即锆石，宝石级、折射率 ~1.95、色散接近钻石；钛铁矿 / 金红石深色不透光。
- **登录页（`e907f61`）**：`components/OrbitStations`（插画待机压暗、只亮中枢；每切到一个没切过的系统点亮一个基站：同图亮版 + 柔边椭圆遮罩放大露出、光晕 / 脉冲环 / 光柱、数据光弧 + 光点射向中枢、标签逐字解码、「节点接入 n / 07」读数；全部切过 → 剩余依次点亮 → 光点齐射 → 压暗撤掉 + 冲击波 + 光柱 → 晶体升起）；`components/OrbitGem`（Three.js：八方柱 + 双锥锆石晶形、透射 + ior 1.95 + 色散、背面深蓝镜面内层、棱线、中心辉光；**去掉点光**——平整刻面上的点光高光像两只眼睛，闪光只靠环境反射）。three 加进依赖并单独分包 `vendor-three`（gzip 184KB），只在登录页动态加载、切到第 2 个系统后空闲预取；首轮构建它被并进公共 `vendor`（每页都会下），`vite.config` 加分包规则后 vendor 回到 302KB。
- **数据看板（`3404f8f`）**：新指令 `v-decode`（`directive/common/decode.ts`，数字位乱跳后逐位定格，一批错开 45ms，`replayDecode` 刷新后值没变也重算）；进页面 / 刷新时扫描光 + 面板四角弧线依次亮；标题栏「数据链路 · 实时同步」信号条 + 底边数据轨（12s 三颗光点）；阶段条 12s 流光；到港图柱子依次长出、货值峰值打 4s 波纹。
- **业务页（`aa7d6d8`，只播一次）**：`ListTableCard` 每来一批数据行依次浮现 + 淡青光退掉（只动透明度 / 内阴影，固定列不错位）；加载遮罩扫描线；`DetailHeaderCard` 标题和字段值、海关数据统计卡数字 `v-decode`（解码时发青光，数字等宽）；NProgress 青蓝数据流。
- **踩坑**：① `v-decode` 起初拿 DOM 文字比对判断「Vue 有没有渲染新值」，乱跳的那一帧碰巧等于新值时会定格成旧值（实测「9%」定格成「0%」）；改成取 vnode 的渲染文字——带自定义指令的元素子节点是一组文本 vnode，按 `child.el === 文字节点` 取对应那个（值后面跟单位 `<span>` 也对）。② 渲染台设深色要在 `DOMContentLoaded` 后写 `data-theme`，`addInitScript` 里 `documentElement` 还不存在；渲染台切分支后要重启。③ headless 下 transmission 材质 + 常开 WebGL 截图会超时，用 reducedMotion 单帧或不加 GL 参数。
- 验证：渲染台截图（登录 1920 逐步 1 / 3 / 6 / 全亮 / 减少动态；看板深浅两套、刷新重播、三次跑数值全对；业务页演示页加载 / 到达 / 定格）；三次生产构建通过；推后核对 `origin/feature-v1.8.4` 不含深空提交。未在真实浏览器 / GPU 上看过晶体（headless 是软件渲染）。

### 2026-09-24：详情页统一 / 带鱼屏 / 底栏 / 个人中心合入深空分支（`1.8.4-deep-space-theme`，`4b31d60` + `11391a8`）
- 单向合 `origin/feature-v1.8.4` → 深空（`d065a25`…`c5d22dc`），**无冲突**；只推深空，推后核对 feature 不含深空提交、本地切回 `feature-v1.8.4`。
- 深色核查（渲染台，重启后）：个人中心、仓库新增、消息设置、提单新增、开票新增（1792 与 2560）都正常；深空样式里引用被删类名的只剩个人中心旧 `list-group-item`（已无元素，规则闲置无害）和小程序用户详情 `.app-container:has(.id-text)`（该页根节点未动，仍生效）。
- **深空独有问题一处，已修**：深空把必填「*」换成 5px 红点 + 间距 7（共 12），浅色「*」是 6 + 4（共 10）；详情页标签区 100（文字可用 88），6 字必填标签 78 + 12 = 90 超 2px，深色下「提单发票金额」「纳税人识别号」折行且点和字分开。`deep-space-neon.scss` 间距 7 → 5（右置时 margin-left 同改），实测折行消失。只在深空分支。
- 验证：深空分支生产构建通过（`dist` 已删、`auto-imports.d.ts` 已还原）。

### 2026-09-24：带鱼屏 4 格 / 按钮统一进底栏 / 个人中心改造（`feature-v1.8.4`，`e69dc9b` / `b15b058` / `c5d22dc`）
- **带鱼屏**（用户 / 设计：正常屏一行 3 个，带鱼屏最多 4 个，宽度、间距一致）：皮肤给 `.collapse-content-box` 设 `container-type: inline-size`，内容区 ≥ 2344px（`$zk-form-four-columns`）时 `.el-col-8` → 25%（排除弹框 / 抽屉里的行），非栅格项与国内采购 / 销售自建网格同一门槛。门槛推导：4 格框宽 ≥ 1920 屏 3 格框宽 471。实测 1920：3 格 471；2560：4 格 484；3440：4 格 704；间距仍 20。⚠️ 渲染台 vite 没拾到全局 scss 改动（样式表里没有新规则），重启渲染台后才生效——以后改完全局样式实测前先确认规则进了页面。
- **按钮进悬浮底栏**（用户：很多页面下方有按钮，都按悬浮底部按钮组件的方式）：`.codex-tmp/minerals/btn_survey.py` 找出 41 个详情类页面（表单末尾 / 标题行放「取消 / 提交 / 返回…」、不在弹框 / 表格 / 已有底栏里）；`btn_bottom.py` 自动换 34 个（找只装按钮的最小外层 → `BottomFixedBtnsBox`，保留外层 v-if，返回 / 取消放最前、去 type 颜色和内联样式、统一 size large，补 import 与根节点 `form-have-bottom-btn`），7 个只读详情的「返回」手工换；清掉不再用的 `detail-actions` / `zk-*-footer` / `form-actions` 样式；消息设置页标题行给按钮写死 78×30 的样式一并删掉。资讯 / 提货 / 帮助中心三个只读详情原来连 `zk-detail-page` 都没挂（底栏按钮 40 高），已补。渲染台复测 41 页底栏按钮全部 38 高。DESIGN_SPEC 6.6 补「页面级按钮一律进底栏」。
- **个人中心**（用户：给点间距、圆角，按其它详情页风格）：去掉 `app-container` 白底，改两张区块卡网格（左 340「个人信息」：头像 96 + 昵称 + 提示、信息行 44 高分隔线、标签辅助色在左值在右；右「账号设置」：页签）；基本资料 / 修改密码表单改标签在左 100、一行 3 格；保存 / 关闭进各自页签内的底栏（页签隐藏时底栏跟着隐藏，实测只显示当前页签的），「关闭」去掉红色 danger。不加顶部信息卡（规范 6.1 只给只读详情）。
- 验证：改动 SFC 编译通过；三次生产构建通过；`dist` 已删、`auto-imports.d.ts` 已还原。浅色下渲染台截图核对；深空分支未合。

### 2026-09-24：详情页表单统一成新增开票页版式（`feature-v1.8.4`，`15e64e4`）
- 用户：所有详情页的下拉框、输入框、时间框长度高度统一，以新增开票页「基础信息」为标准；复合框（提单发票金额）整组宽度和其它一致。
- 标准实测（渲染台 1792 宽）：标签在左、自适应出来 100、一行 3 格（gutter 20 / span 8）、框 429 × 40。实测前 50 个新增页高度已全是 40，宽度 150–529 不等；根因是两套版式（约 40 页「标签在上」一行 3 或 4 格 + 若干写死宽度），问用户后定为**全部改成开票页版式**（放弃 Figma 的上下结构，DESIGN_SPEC 4.2 / 11.11 同步）。
- 实施：
  - 脚本（`.codex-tmp/minerals/form_survey.py` / `form_unify.py`，跳过弹框 / 抽屉里的表单、列表搜索表单、登录 / 监控 / 代码生成等非详情页）改 73 个表单：去 `label-position="top"`、`label-width="100px"`、span 6→8、gutter→20；融资 span 12、结算 span 10 的输入框改 8；
  - 超过 6 字的标签在 40 高内折两行（皮肤 `white-space: normal` + 行高 18）；两行放不下的 4 个附件长标签该项单独 `label-position="top"`（按像素宽判断，一行约 88px）；试过 `text-wrap: balance`，必填星号会和文字分开，撤回；
  - 组合框：皮肤新增 `zk-combo-main`（主输入吃剩余宽度、整组 nowrap gap 8），用在提单发票金额、平台推荐金额、请选择仓库、上级品类、帮助中心颜色；
  - 不在栅格里、直接放区块卡的表单项：`.collapse-content-box > .el-form-item` 用 container query 把内容区限成一格框宽（(100cqw+20)/3−120）；
  - 国内采购 / 销售自建网格 gap 24→20；角色、用户组两页的自建「440/240 定宽 + 上方标签」字段区改成 el-row/el-col；码头 / 堆场提货新增补 `zk-detail-page`（原来连 40 高都没有）；国内销售 script 在 template 前被首轮脚本漏掉，已补；帮助中心根节点 `app-container` 多一层白底 + 16 内边距，已去掉（后续提交），框回到 429、颜色组合框整组 429。
- **同事并行改动**：推送时远端多了 kimi 17:09 `d065a25`（码头 / 堆场提货新增、详情大改，新增页做成「标签在上、一行 4 格」）。变基时冲突文件取同事新版，再在新版上套同一套统一转换（两页实测 429 × 40）；详情页是只读标签值，未动。需要告诉用户：同事的「标签在上」被按用户口径改成了左标签。
- 验证：复测全部新增页（除特殊页）为 429 × 40、组合框整组 429；69 个改动 SFC 编译通过；两次生产构建通过；`dist` 已删、`auto-imports.d.ts` 已还原。只在浅色下看过；深空分支未合（用户没要求）。
- 例外：消息设置页嵌在句子里的小数字框（150）、仓库 / 监管仓的堆场逐行编辑器、整行文本框 / 附件 / 表格保持原宽；纯只读详情（项目、台账、贷后、小程序用户、提货详情）没有输入框。
