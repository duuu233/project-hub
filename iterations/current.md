...
- **小程序：`node tests/*.test.js` 59 个文件全过（真跑）**，`ai-dialogue-guard` 新增两条
  （弹窗里的数字跟着账户接口走、账户接口失败时数字不被抹掉）。
- **APP：本机无 Flutter 工具链**，analyze/test/编译/真机全没跑，新增的 l10n 用例也没跑过；
  借 flowerpot 的四个 Dart 静态自检跑了 166 个文件全绿。
- ⚠️ 两端都没在真机/开发者工具上看过版式；iOS 上「那颗按钮确实不出现」也还没验过。
