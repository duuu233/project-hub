...
- 用户：「按花盆的统一，离线也拦，升级中打开进度弹层」「能复用的就复用，包括逻辑和交互」，并补充 4 点口径：割草机当前版本读 DP113 `fw_version`；升级中再点入口打开进度弹层（花盆同步割草机）；卡住后弹「升级似乎已中断」（割草机同步花盆）；离线提示「花盆已离线」（花盆同步割草机）；两种提醒弹窗点「立刻更新」都跳到设置页。
